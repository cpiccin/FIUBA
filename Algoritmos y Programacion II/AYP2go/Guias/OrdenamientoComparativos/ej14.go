...
func main() {
	a1, a2, a3, a4, a5, a6, a7 := []int{2,2,2}, []int{8,4,6}, []int{4,5,6}, []int{1,2,3}, []int{9,8,3}, []int{4,5,4}, []int{9,9,2}
	...
	// a5 y a7 tienen mismo promedio distintas notas, ver si estan bien ordenados segun notas individuales
...
func CountingSort(arr []alumnos, k int, nota string) []alumnos {
	count := make([]int, k)
...
func pos(i alumnos, nota string) int {
	if nota == "promedio" {
		return i.Promedio()
	} else if nota == "1" {
		return i.Nota(1)
