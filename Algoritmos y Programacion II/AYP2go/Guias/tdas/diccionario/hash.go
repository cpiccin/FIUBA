...
func (self *hash[K, V]) buscarPosibleIndiceDeClave(clave K) (bool, int) {
...
func (self *hash[K, V]) Pertenece(clave K) bool {
	res, _ := self.buscarPosibleIndiceDeClave(clave)
	return res
}

func (self *hash[K, V]) Obtener(clave K) V {
	res, index := self.buscarPosibleIndiceDeClave(clave)
	if !res {
