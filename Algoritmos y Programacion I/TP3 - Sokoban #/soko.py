...
def define_movimientos(grilla, direccion, c, f):
    ...
    if hay_objetivo(grilla, c, f) and hay_caja(grilla, c1, f1):
        ...
        if hay_objetivo(grilla, c1, f1):      
            grilla[f2][c2], grilla[f1][c1], grilla[f][c] = CAJA_FUERA_OBJETIVO, OBJETIVO_Y_JUGADOR, OBJETIVO
            return
        grilla[f2][c2], grilla[f1][c1], grilla[f][c] = CAJA_FUERA_OBJETIVO, JUGADOR, OBJETIVO
        return 
    ...
    if hay_objetivo(grilla, c1, f1):
        if hay_objetivo(grilla, c, f) and hay_jugador(grilla, c, f):
            grilla[f1][c1], grilla[f][c] = OBJETIVO_Y_JUGADOR, OBJETIVO
            return      
