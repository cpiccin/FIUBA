...
def movimiento_valido(grilla, direccion, c, f):
    ...
    if (hay_caja(grilla, c1, f1) and hay_pared(grilla, c2, f2)) or (hay_caja(grilla, c1, f1) and hay_caja(grilla, c2, f2)) or hay_pared(grilla, c1, f1):
        return False
    return True
...
def define_movimientos(grilla, direccion, c, f):
    ...
    c1, f1 = c+direccion[0], f+direccion[1]
    c2, f2 = c1+direccion[0], f1+direccion[1]
    if hay_objetivo(grilla, c, f):      #si hay un objetivo y jugador en la misma posicion
        if hay_caja(grilla, c1, f1):
            grilla[f][c], grilla[f1][c1], grilla[f1][c2] = '.', '@', '$'
            return
        grilla[f][c], grilla[f1][c1] = '.', '@'
        return
    if hay_caja(grilla, c1, f1):        #si hay una caja en el casillero siguiente
        if hay_objetivo(grilla, c1, f1):
            if hay_objetivo(grilla, c2, f2):
                grilla[f][c], grilla[f1][c1], grilla[f2][c2] = ' ', '+','*'
                return
            grilla[f][c], grilla[f1][c1], grilla[f2][c2] = ' ', '+', '$'
            return
        if hay_objetivo(grilla, c2, f2):
        ...
        grilla[f][c], grilla[f1][c1], grilla[f2][c2] = ' ', '@', '$'
        return
    if hay_objetivo(grilla, c1, f1):    #si hay un objetivo en el casillero siguiente
        grilla[f][c], grilla[f1][c1] = ' ', '+'
        return
    grilla[f][c], grilla[f1][c1] = ' ', '@'
    return
...
def mover(grilla, direccion):
    ...
    for x in grilla:
        ...
        for e in x:
            fila_x.append(e)
        grilla_actual.append(fila_x)    #para que al final no se modifique la grilla del principio
    c, f = posicion_jugador(grilla_actual)
    if movimiento_valido(grilla_actual, direccion, c, f):
